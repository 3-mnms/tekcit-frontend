...
export default function NearbySpotEmbed({
  ...
  useEffect(() => {
    ...
    const load = () => {
      ...
      // course 탭: 공연장만
      const targets: Array<{ id?: string; name?: string; lat: number; lng: number; label?: string }> =
        active === 'course'
    ...
    <section className={styles.page} aria-label="주변 추천">
      ...
      {isError && (
        <div className={styles.error}>
          불러오기에 실패했어요. <button className={styles.retry} onClick={() => refetch()}>다시 시도</button>
        </div>
      )}
      {!isLoading && !isError && !rec && <div className={styles.empty}>해당 공연장의 추천 데이터가 아직 없어요.</div>}
