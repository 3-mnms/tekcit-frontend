...
// ✅ /qr 호출용 API (프로젝트 alias에 맞춰 경로 확인)
import { apiReserveTicket, type IssueQrRequest } from '@api/booking/bookingApi'
...
const TicketOrderInfoPage: React.FC = () => {
  ...
  // 예약번호 확보: state > query(resNo) > session
  ...
  // 라디오 변경 → QR은 즉시 저장, PAPER는 결제 시 저장
  ...
  // 결제 이동 (클릭 시점에 최신 주소/수령방법 저장 + QR(티켓생성) 호출)
  const handlePay = async () => {
    ...
    // 1) 결제 직전, 현재 method로 항상 서버 저장(덮어쓰기) → QR도 null 방지
    try {
    ...
    // 2) 결제 직전, 예매 티켓 생성(/qr) 호출 — method와 무관하게 호출(서버가 PAPER도 동일 엔드포인트 처리)
    try {
      const req: IssueQrRequest = {
        festivalId: fid,
        reservationNumber: reservationNumber!,
        performanceDate: perfDateISO, // 예: "2025-09-23T17:00:00"
      }
      await apiReserveTicket(req)
    } catch (e) {
      console.error('[reserveTicket(/qr)] failed', e)
      alert('발권(예매 티켓 생성)에 실패했어요. 잠시 후 다시 시도해 주세요.')
      return
    }

    // 3) 결제 페이지 이동 (세션 백업 포함)
    ...
    <div className={styles.page}>
      ...
      <div className={styles.rightCol}>
        ...
        <OrderConfirmSection
          ...
          reservationNumber={reservationNumber!}
          bookerName={user?.name ?? ''}
          onPay={handlePay} // ✅ 내부 API 호출 스킵, 페이지에서 전부 처리
        />
      </div>
    </div>
  )
